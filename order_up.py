class OrderUp:
    ...
    def check_if_on_menu(self, item, category):
        ...
        return item_lower in (key.lower() for key in self.menu.get(category, {}).keys())
    ...
    @staticmethod
    def calculate_total(menu, order_items):
        total = 0
        for category, items in order_items.items():
            for item in items:
                if item in menu.get(category, {}):
                    total += menu[category][items]
        return total
